...
void prox_l1_norm_active_set( Matrix *X, Matrix *V, double lambda ) {
	...
	indexed_double *u = malloc(sizeof(indexed_double)*X->ncols);
	...
	if(t_lb < 0) {
		...
		double *umax = malloc(sizeof(double)*M);
	...
	char *J = malloc(sizeof(char)*X->nrows*M);
	double *psum = malloc(sizeof(double)*M);
	int *nzJ = malloc(sizeof(int)*M);
	for(ii=0;ii<M;ii++){
		nzJ[ii] = X->nrows;
		psum[ii]= u[ii].val;
	}
	...
	// variables
	double t_new, w;
	char *update_Ji = malloc(sizeof(char)*M);
	char J_converged;
...
void prox_l1_norm_column_sort( Matrix *X, Matrix *V, double lambda ) {
	...
	double *U = malloc(sizeof(double)*V->nrows*M);
	for(ii=0;ii<M;ii++) {
		// copy data
	...
	// create variables
	double *psum = malloc(sizeof(double)*M); 	// store partial sums
	int    *nj	 = malloc(sizeof(int)*M);		// store number of non-zero elements
	// initialize variables
	for(ii=0;ii<M;ii++){
...
Matrix *create_matrix(int nrows, int ncols) {
	...
	A->data = malloc(sizeof(double) * nrows * ncols );
	// assigm dimensions
	A->ncols = ncols;
	A->nrows = nrows;
	// return value
